...
This is the base subscription event that all the subsequent events will be based of off. All events following this one will use these parameters **in addition** to whatever other parameters are shown in the JSON template. Active (Subscribed) is the steady state of a provisioned SaaS subscription. After the Microsoft side has processed the Activate Subscription API call, the SaaS subscription is marked as Subscribed. The customer can now use the SaaS service on the publisher's side and will be billed.
...
operationId | unique identifier (GUID) that is attached to each market place operation, generated by the market place API
subscription | --
...
isTest| true when running test mode, false otherwise
isFreeTrial | is it a free trial
status | 
term | --The period for which the subscription was purchased.
termUnit|  where P1M is monthly and P1Y is yearly. Also reflected in the startDate and endDate values
startDate |format: YYYY-MM-DD. This is the date when the subscription was activated by the ISV and the billing started. This field is relevant only for Active and Suspended subscriptions.
endDate | This is the last day the subscription is valid. Unless stated otherwise, the automatic renew will happen the next day. This field is relevant only for Active and Suspended subscriptions.
